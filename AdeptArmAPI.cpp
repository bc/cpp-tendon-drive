...
// Function Move
void AdeptArmAPI::movetrans (EPoint displacement) {
...
void AdeptArmAPI::moveInCircleTrans(double radius, int numberOfPoints) {
    ...
    for (int i = 0; i <= numberOfPoints; i++) {
        ...
        std::cout << "Current: " << current.X << "," << current.X << std::endl;
        std::cout << "This point " << displacementFromDefaultX << "|" << displacementFromDefaultY << std::endl;
        movetrans(EPoint(displacementFromDefaultX, displacementFromDefaultY, 0, 0, 0, 0));
    }
...
// Function set origin
void AdeptArmAPI::setOrigin (EPoint position) {
    origin = position;
}
